...
- Quota 시계열 히스토리 저장 및 카드 내 미니 트렌드(스파크라인) 제공
- `즉시 조치` 섹션으로 위험 세션 우선 노출(로그인 필요/한도 임계/지연)
- 세션 카드에 위험 원인 라벨과 상황 맞춤 1차 액션(로그인/새로고침/열기) 적용
- 상태 판정 안정화(짧은 시간 내 비핵심 상태 깜빡임 억제)
- 알림 쿨다운(90초) 적용으로 짧은 시간 반복 알림 노이즈 감소
